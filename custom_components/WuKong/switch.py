...
def setup_platform(hass, config, add_devices_callback, discovery_info=None):
    ...
    hass.services.register(DOMAIN, 'WuKong_Send_Control_Command', service.SendControlCommand)
    hass.services.register(DOMAIN, 'WuKong_Send_Open_Command', service.SendOpenCommand)
    hass.services.register(DOMAIN, 'WuKong_Send_Install_Command', service.SendInstallCommand)
    hass.services.register(DOMAIN, 'WuKong_Send_Clean_Command', service.SendCleanCommand)
    hass.services.register(DOMAIN, 'WuKong_Send_Command_Queue', service.SendCommandQueue)
    hass.services.register(DOMAIN, 'WuKong_Send_Connect_Command', service.SendConnectCommand)
    hass.services.register(DOMAIN, 'WuKong_Send_Push_File', service.PushFile)
...
class WuKongService(object):
    ...
    def PushFile(self,call):
        host = call.data.get('host')
        src = call.data.get('src')
        url = 'http://{host}:12104/?action=intent&f2t=vc&ai=false'.format(host=host)
        return self.pushHttpRequest(url,src)
    ...
    def pushHttpRequest(self,url,src):
        data ='{"p":"com.wukongtv.wkhelper","n":"iqiyi","u":"","vf":"iqiyi","is":[{"m":1,"i":"#Intent;launchFlags=0x10000000;component=com.wukongtv.wkhelper/com.wukongtv.wkhelper.pushscreen.PushScreenNewActivity;S.v='+src+';S.n=chenqu;i.w=274;end"}]}'
        try:
